...
const loadData = () => {
    ...
    if (tempItemData !== null) {
        ...
        items.forEach(response => {
            itemOptions += `<option value="${response.itemCode}">${response.itemCode}</option>`;
        });
...
function setItemData() {
    ...
    let tempItem = items.find(response => response.itemCode == tempItemCode);
...
const updateItemQty = (details) => {
    ...
    details.forEach(responseData=>{
        if (itemArr !== null) {
            for (let i = 0; i <itemArr.length; i++) {
                if (itemArr[i].itemCode==responseData.code){
                    console.log(itemArr[i].qtyOnHand);
                    itemArr[i].qtyOnHand = (itemArr[i].qtyOnHand-responseData.qty);

                    console.log(itemArr[i]);
                    console.log(isNaN(responseData.qty));
                    console.log(isNaN(responseData.qtyOnHand));
                    // console.log(itemArr[i].qtyOnHand);
                    break;
                }
            }
    ...
    function removeItem(code) {
        if (confirm("Are you sure?")) {
            for (let i = 0; i < cart.length; i++) {
                if (cart[i].code === code) {
                    cart.splice(i, 1); // delete cart item
                    setCartData();
                }
            }
        }
